...
const HEADINGS: Record<Lang, { label: string; heading: string; subtitle: string }> = {
  English: {
    ...
    subtitle: "Hackathon winners, coursework, and things built because they were interesting.",
  },
  French: {
    label: "Projets",
    heading: "Travaux choisis",
    subtitle: "Projets primés en hackathon, travaux de cours, et créations par curiosité.",
  },
  Spanish: {
    label: "Proyectos",
    heading: "Trabajos seleccionados",
    subtitle: "Ganadores de hackathon, trabajos de curso y proyectos por curiosidad.",
  },
};
...
function Projects({ language }: LanguageProps): JSX.Element {
  ...
  // Award-winning work leads and gets the wide, featured treatment.
  const ordered = [...PROJECTS].sort(
    (a, b) => Number(Boolean(b.award)) - Number(Boolean(a.award)),
  );

  return (
    <section id="projects" className={`${section.band} ${section.bandAlt}`}>
      <div className={section.inner}>
        <SectionHeader label={label} heading={heading} subtitle={subtitle} />

        <ul className={styles.grid}>
          {ordered.map((project: Project, i) => {
            const desc =
            ...
            const award =
              lang === "French" && project.award_fr
                ...
                : lang === "Spanish" && project.award_es
                  ? project.award_es
                  : project.award;
            const featured = Boolean(project.award);
